...
`backup1.rma.cloudscale.vshn.net`: VSHN AG: cloudscale.ch AG: e-shelter datacenter, Rümlang, Switzerland
`backup2.rma.cloudscale.vshn.net`: VSHN AG: cloudscale.ch AG: e-shelter datacenter, Rümlang, Switzerland
...
* All data is encrypted (using Blowfish algorithm) on the client and the encrypted data is then sent to the backup server
